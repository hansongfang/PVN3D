...
if __name__ == "__main__":
    ...
    if not args.eval_net:
        ...
        #         num_workers=10
        #     )
        logger.info(f'Ignore evaluation!')
    ...
    cur_mdl_pth = os.path.join(log_model_dir, 'pvn3d.pth.tar')
    ...
    def bnm_lmbd(it): return max(
        args.bn_momentum
        * args.bn_decay ** (int(it * mini_batch_size / args.decay_step)),
        bnm_clip,
    )
    ...
    if args.eval_net:
        ...
        trainer.train(
            it, start_epoch, n_total_epoch, train_loader, val_loader,
            best_loss=best_loss
        )

        if start_epoch == n_total_epoch:
